...
class Alphabet:
    def __init__(
        self, ipa_file="../data/ipa.txt", segs=None, add_segs=False, nas_vowels=False
    ):
        self.segments = set()
        ...
        with open(f"{dir_path}/{ipa_file}", "r", encoding="utf-8") as f:
    ...
    def _add_or_remove_feats(self, seg, feats, add=True):
        ...
        if type(feats) is str:
            feats = (feats,)
        if not add and not all(
            seg.feature_vec[self.feature_space.index(feat)] for feat in feats
        ):
            return None
        ...
        if new_feat_vec not in self:
            return None
        if new_feat_vec == list(
            seg.feature_vec
        ):  # if the vector is unchanged, return None
            return None
    ...
    def set_feats(self, seg, feats, vals):
        ...
        if len(feats) != len(vals):
            raise ValueError(
                f"Length of :feats: and :vals: must be equal, but are |feats| = {len(feats)} and |vals| = {len(vals)}"
            )
        seg = self[seg]
        new_feat_vec = list(seg.feature_vec)
        ...
        if exclude_unspec:
            return set(
                filter(
                    lambda feat: feat[0] != "?",
                    set(
                        f"{seg.feature_vec[i]}{feat}"
                        for i, feat in enumerate(self.feature_space)
                    ),
                )
            )
        return set(
            f"{seg.feature_vec[i]}{feat}" for i, feat in enumerate(self.feature_space)
        )
    ...
    def shared_feats(self, segs, exclude_unsepc=True):
        ...
        return set.intersection(
            *list(self.feat_vals(seg, exclude_unspec=exclude_unsepc) for seg in segs)
        )
