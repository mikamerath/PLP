...
class NatClassGen:
    ...
    def induce_nat_classes(self, r, ngrams):
        ...
        for idx, seg in enumerate(r.CAD()):
            if (
                seg in {"#", "."} or "#" in seg or "." in seg
            ):  # we cannot induce a natural class over word or syllable boundaries
                _seq.append(seg)
            elif (
                self.skip_gen_A and idx == target_index
            ):  # if skip_gen_A = True, don't induce a nat class over the target (This is just an optimization for the locality experiment that does not affect the result).
                _seq.append(seg)
        ...
        if not success:
            ...
            es = list(
                filter(lambda ngram: seq.matches(ngram[0]) and not ngram[-1], ngrams)
            )
            # print(es)
            return r
        ...
        for seq_seg, r_seg in zip(seq, new_r.CAD()):
            if len(seq_seg) == 0:  # make sure every seg has at least one feature
                options = sorted(
                    self.alphabet.shared_feats(r_seg),
                    key=lambda feat: (len(feat_to_segs[feat]), feat),
                )
